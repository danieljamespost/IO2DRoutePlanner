...
void RoutePlanner::PopulatePath(RouteModel::Node *current, std::vector<RouteModel::Node> *path)
{
    this->distance += current->distance(*(current->parent));
 	RouteModel::Node *parent = current->parent;
	path->insert(path->begin(), *parent);  
    if (this->isAtStart(parent)) return;
    this->PopulatePath(parent, path);
}

std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *ending_node)
{
...
void RoutePlanner::AStarSearch()
{
  ...
  while (this->open_list.size() > 0) {
  	...
  	if (this->isAtEnd(current)) this->m_Model.path = ConstructFinalPath(this->end_node);
